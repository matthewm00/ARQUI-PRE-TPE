...
void putChar(char c)
{
    syscall(SYS_WRITE_ID, (uint64_t)&c, 1, currentBGC, currentFTC);
}
...
static int commandLine(char *str)
{
    ...
    {
        syscall(SYS_WRITE_ID, (uint64_t) "$", 1, BLACK, GREEN);
        syscall(SYS_WRITE_ID, (uint64_t) ">", 1, BLACK, GREEN);
        putChar(' ');
        return 1;
    }
...
void printf(char *str, ...)
{
    ...
    syscall(SYS_WRITE_ID, (uint64_t)buff, buffIdx, currentBGC, currentFTC);
    va_end(args);
    return;
}
...
char getChar()
{
    return syscall(SYS_READ_ID, 0, 0, 0, 0);
}
